/**
 * Página CRUD para gestión de Fondos de Cesantías
 * Permite crear, editar, eliminar y listar fondos de cesantías
 */
...
export default function CesantiasPage() {
  const [fondos, setFondos] = useState<FondoCesantias[]>([])
  ...
  const [selectedRecord, setSelectedRecord] = useState<FondoCesantias | null>(null)
  ...
  const loadData = async () => {
    ...
    // Check cache first
    const cached = localStorage.getItem('fondos_cesantias_cache')
    if (cached && !dataLoaded) {
      ...
      localStorage.removeItem('fondos_cesantias_cache')
    }
    ...
    try {
      ...
      if (error) {
        console.error('Error loading fondos cesantias:', error)
        showToast('Error al cargar los fondos de cesantías', 'error')
        return
      }
      ...
      // Save to cache
      localStorage.setItem('fondos_cesantias_cache', JSON.stringify({
        data: fondosData,
        timestamp: Date.now()
      }))
  ...
  const handleCreate = async (formData: any) => {
    ...
    if (error) {
      console.error('Error creating fondo cesantias:', error)
    ...
    localStorage.removeItem('fondos_cesantias_cache')
    await loadData()
    showToast('Fondo de cesantías creado exitosamente', 'success')
  }
  ...
  const handleEdit = async (formData: any) => {
    ...
    if (error) {
      console.error('Error updating fondo cesantias:', error)
      throw new Error(error.message)
    }
    ...
    localStorage.removeItem('fondos_cesantias_cache')
    await loadData()
    showToast('Fondo de cesantías actualizado exitosamente', 'success')
  }
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      if (error) {
        console.error('Error deleting fondo cesantias:', error)
        throw new Error(error.message)
      }
      ...
      localStorage.removeItem('fondos_cesantias_cache')
      await loadData()
      showToast('Fondo de cesantías eliminado exitosamente', 'success')
      setShowDeleteModal(false)
      setSelectedRecord(null)
    } catch (error: any) {
      console.error('Error deleting fondo cesantias:', error)
      showToast(error.message || 'Error al eliminar el fondo de cesantías', 'error')
    } finally {
      setDeleting(false)
    }
  }
  ...
  return (
    <div className="p-3 sm:p-6 max-w-7xl mx-auto w-full">
      ...
      <div className="mb-6">
        <div className="flex items-center space-x-4 mb-4">
          ...
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h1 className="text-2xl font-bold text-[#004C4C]">Fondos de Cesantías</h1>
              <p className="text-[#065C5C] text-sm">
                Gestiona los fondos de cesantías disponibles
              </p>
            </div>
          </div>
        </div>
      ...
      <AuxiliaryDataTable
        ...
        columns={columns}
        loading={loading}
        searchPlaceholder="Buscar fondos de cesantías..."
        ...
        canEdit={canEdit}
        canDelete={canDelete}
        emptyMessage="No hay fondos de cesantías registrados"
      ...
      <AuxiliaryTableModal
        ...
        title="Agregar Nuevo Fondo de Cesantías"
        fields={formFields}
      ...
      <AuxiliaryTableModal
        ...
        title="Editar Fondo de Cesantías"
      ...
      <DeleteConfirmModal
        ...
        message="¿Estás seguro de que deseas eliminar este fondo de cesantías?"
