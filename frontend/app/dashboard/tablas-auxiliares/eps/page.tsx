/**
 * Página CRUD para gestión de EPS
 * Permite crear, editar, eliminar y listar EPS
 */
...
export default function EPSPage() {
  ...
  const loadData = async () => {
    ...
    // Check cache first
    const cached = localStorage.getItem('eps_cache')
    ...
    try {
      ...
      if (error) {
        ...
        showToast('Error al cargar las EPS', 'error')
        return
      }
      ...
      // Save to cache
      localStorage.setItem('eps_cache', JSON.stringify({
        data: epsData,
        timestamp: Date.now()
      }))
  ...
  const handleCreate = async (formData: any) => {
    ...
    localStorage.removeItem('eps_cache')
    await loadData()
    showToast('EPS creada exitosamente', 'success')
  }
  ...
  const formFields = [
    {
      ...
      type: 'text' as const,
      required: true,
      placeholder: 'Ej: EPS Sura, Nueva EPS, Sanitas...'
    }
  ]
  ...
  return (
    <div className="p-3 sm:p-6 max-w-7xl mx-auto w-full">
        ...
        <div className="flex items-center space-x-4 mb-4">
          ...
          <div className="flex items-center space-x-3">
            <div className="p-2 bg-pink-100 rounded-lg">
              <Cross className="w-6 h-6 text-pink-600" />
            </div>
            <div>
              <h1 className="text-2xl font-bold text-[#004C4C]">EPS</h1>
      ...
      <AuxiliaryDataTable
        ...
        canEdit={canEdit}
        canDelete={canDelete}
        emptyMessage="No hay EPS registradas"
